...
- AI 경로에는 실제 Supabase secret key를 넘기지 않는다. `apps/api` 환경에는 [계정 탈퇴 서버 경계](account-deletion-server-boundary.md)에 따라 실제 secret key가 있지만, AI 경로는 자격 증명이 아닌 자리표시자를 `env.secretKeys`로 명시해 관리자 클라이언트가 성공하지 못하게 한다.
...
- AI 요청은 `supabaseAdmin`을 사용하지 않고 로그인 사용자의 RLS가 적용되는 `supabase` 클라이언트를 사용한다. 이 클라이언트를 만드는 데 필요한 `SUPABASE_PUBLISHABLE_KEY`는 공개 값이므로 서버 환경에 둔다. 계정 탈퇴 뒤 남은 access token이 비용이 드는 AI 요청을 보내지 못하도록 Supabase Auth에 현재 사용자 또는 세션이 남아 있는지도 확인한다.
- 자리표시자는 자격 증명이 아니라 AI 경로에서 권한을 없애는 값이다. `sb_secret_` 형태를 쓰지 않으며, 누군가 AI 경로의 `supabaseAdmin`을 호출하면 조용히 통과하지 않고 이 값으로 실패한다. 같은 프로세스의 다른 기능이 실제 secret key를 쓰더라도 AI 경로의 관리자 권한은 별도 결정 없이 넓히지 않는다.
...
계정 탈퇴가 같은 Hono 앱에 실제 secret key를 추가하지만 AI 경로가 그 권한을 쓸 이유는 없다. 경로별 자리표시자를 유지하면 AI 요청 문맥에 작동하는 관리자 클라이언트가 들어오지 않는다. 서명만 확인한 JWT는 계정 삭제 뒤에도 만료 전까지 남을 수 있으므로, 비용 보호에는 현재 사용자 또는 세션 확인도 필요하다.
