...
- React Native UI 안에서 HeroUI `BottomSheet`를 route 또는 독립 화면 presentation의 대체물로 사용하지 않는다. 시트를 여는 화면 액션은 Portal overlay를 마운트하지 않고 Native Stack route로 이동한다.
...
화면 단위로 주 렌더러를 선택하면 React Native와 플랫폼 UI 사이의 크기 측정, safe area, 테마와 상태 전달 문제를 화면 곳곳에서 반복하지 않아도 된다. 시트를 Native Stack route로 유지하면 플랫폼의 presentation, dismiss gesture, header와 toolbar를 보존하면서도 본문 렌더러를 화면 목적에 맞게 선택할 수 있다. navigation title도 같은 의미를 공유하되 각 운영체제의 기본 위계로 표현하면 플랫폼별 별도 헤더를 만들지 않아도 된다. universal Settings는 하나의 트리로 각 운영체제의 네이티브 UI를 얻는다. 반면 `GlassView`와 `BlurView`는 React Native 트리 안에서 사용하는 API이므로, 필요한 플랫폼 표현을 얻으면서 화면 소유권은 React Native에 유지할 수 있다.
...
- navigation이나 toolbar 의미가 전혀 없는 동일 화면의 임시 패널이 필요하고 Native Stack route로 표현하면 사용 흐름이 실제로 악화된다.

## Still-rejected alternatives

- HeroUI `BottomSheet`로 독립 화면이나 route-level sheet를 표시 — React Native Portal overlay라 Native Stack의 기본 header, `Stack.Toolbar`, route 전환과 presentation 소유권을 잃는다. navigation 의미가 없는 동일 화면의 임시 패널 요구가 확인될 때만 다시 검토한다.
