...
- `@expo/ui 57.0.9`의 Android `Switch label`은 보이는 라벨과 스위치를 별도 접근성 노드로 내보내고 라벨 색상 API를 제공하지 않는다. 앱은 이 한계를 고치기 위한 패치나 플랫폼별 Settings 트리를 두지 않고 공통 기본 `Switch`를 유지한다. `@expo/ui`를 올릴 때 동작이 바뀌었는지 다시 확인한다.
