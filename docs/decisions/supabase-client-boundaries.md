...
- `apps/mobile/.env.local`은 Git에서 제외하고, 저장소에는 필요한 변수 이름만 보여주는 example env 파일을 둔다.
...
스키마에서 생성한 타입은 동일한 Supabase API 계약을 표현하므로 모바일과 서버가 공유해야 한다. 반면 runtime SDK, session storage, 환경 변수와 비밀 키는 실제로 client를 생성하는 소비자가 소유해야 한다. types-only package로 시작하면 스키마 계약 중복을 막으면서 아직 존재하지 않는 서버를 위해 factory 추상화를 미리 만들지 않는다. 환경 파일 자동 전환은 편의 command와 상태를 추가하고 local 실행이 사용자가 설정한 원격 값을 덮어쓸 수 있다. 초기에는 짧은 수동 절차가 더 명시적이고 안전하다.
