...
- 대화는 Home이 아니라 루트 스택의 `/chat` 화면이 소유한다. Home은 시작 화면만 두고, 헤더 도구 막대의 새 대화 버튼이 대화 화면을 민다. 대화는 그 화면이 스택에 있는 동안만 살아 있으므로 새 대화에 확인 Alert가 필요 없다.
- 대화 화면의 머리글은 뒤로 가기와 제목뿐이고 하단 탭바는 보이지 않는다. 탭 안에서 `NativeTabs`의 `hidden`으로 탭바를 감추면 Android에서 비운 자리가 남아 그 위의 터치를 삼키므로, 루트 스택에서 밀어 네이티브 push가 탭바를 덮게 한다.
