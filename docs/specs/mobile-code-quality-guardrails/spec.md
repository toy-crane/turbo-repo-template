...
- Expo 권장 버전과 다른 패키지를 `expo.install.exclude`에 추가하지 않는다.
- 권장 버전에서 실제 회귀가 iOS와 Android Development Build에 재현될 때만 기존 버전 복구와 예외 등록을 다시 결정한다.
...
6. `react-native-keyboard-controller`는 `1.21.9`로 설치되고 `expo install --check`가 버전 차이 없이 끝난다.
...
- `react-native-keyboard-controller` 외의 패키지 버전은 호환 검사가 새 차이를 찾지 않는 한 바꾸지 않는다.
