...
- Verification: 최종 HEAD의 깨끗한 archive에서 `bun install --frozen-lockfile`; `bun run check`; `bun run check-types`; `bun run test` (11 suites, 17 tests); `bunx expo install --check`; `bun run build --filter=@repo/mobile`을 통과했다. Android Development Build는 Gradle `assembleDebug`가 511개 task를 처리해 `BUILD SUCCESSFUL`로 끝났고 APK 설치와 Metro bundle도 성공했다. Android autolinking `resolve --platform android --json`은 23개 native module을 패키지별 한 번씩 선택했다.
